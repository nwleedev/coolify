...
export default function CoverLetterModal() {
  const { state, onClose } = useCoverLetterModal();
  const windowRef = useRef(null as Window | null);
  const [, setStoredLetter] = useLocalStorage<string>("app:previousLetter");
      ...
      <div className="w-full flex flex-col h-full gap-y-2 px-2">
        <div className="flex flex-col w-full tracking-wide">
          ...
          <p className="text-sm text-gray-500 text-center">
            Here is the improved cover letter.
          </p>
        </div>
        <Textarea
